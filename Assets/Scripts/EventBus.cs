...
public class EventBus
{
    ...
    public static void Publish<T>(T published_event)
    {
        ...
        //if (DEBUG_MODE) Debug.Log("[Publish] event of type " + t + " with contents (" + published_event.ToString() + ")");
        ...
        if (_topics.ContainsKey(t))
        {
            ...
            //if (DEBUG_MODE) Debug.Log("..." + subscriber_list.Count + " subscriptions being executed for this event.");
    ...
    public static Subscription<T> Subscribe<T>(Action<T> callback)
    {
        ...
        //if (DEBUG_MODE) Debug.Log("[Subscribe] subscription of function (" + callback.Target.ToString() + "." + callback.Method.Name + ") to type " + t + ". There are now " + _topics[t].Count + " subscriptions to this type.");
    ...
    public static void Unsubscribe<T>(Subscription<T> subscription)
    {
        ...
        //if (DEBUG_MODE) Debug.Log("[Unsubscribe] attempting to remove subscription to type " + t);

        if (_topics.ContainsKey(t) && _topics[t].Count > 0)
        {
            _topics[t].Remove(subscription);

            //if (DEBUG_MODE) Debug.Log("...there are now " + _topics[t].Count + " subscriptions to this type.");
        }
        else
        {
            //if (DEBUG_MODE) Debug.Log("...but this subscription is not currently valid (perhaps you already unsubscribed?)");
        }
    }
}
